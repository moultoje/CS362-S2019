...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case steward:
      return stewardEffect(state, currentPlayer, choice1, choice2, choice3, handPos);
...
int adventurerEffect (struct gameState *state, int currentPlayer)
{
    ...
    // Loop while the player hasn't drawn two treasure cards.
    while(drawntreasure < 2)
    {
        ...
        else
        {
            ...
            state->handCount[currentPlayer]--; // just remove the most recently drawn card
            z++;
        }
    }
...
int feastEffect (struct gameState *state, int currentPlayer, int choice1)
{
    ...
    for (i = 0; i <= state->handCount[currentPlayer]; i++)
    {
        ...
        state->hand[currentPlayer][i] = -1; // set the backed-up card to nothing
    }
...
int baronEffect (struct gameState *state, int currentPlayer, int choice1)
{
    ...
    if (choice1 > 0) // Boolean true or discard an estate
    {
        int p = 0; // iterator for hand
        int card_not_discarded; // flag for discarded set
        while (card_not_discarded)
        {
            if (state->hand[currentPlayer][p] == estate) // found estate
            {
                state->coins += 4; // Add 4 coins
    ...
    else
    {
        if (supplyCount(estate, state) > 0)
        {
            gainCard(estate, state, 0, currentPlayer); // gain estate
            state->supplyCount[estate]--;
            //if (supplyCount(estate, state) == 0)
            //{
            //    isGameOver(state);
